...
namespace NDCWeb.Areas.Member.Controllers
{
    ...
    public class AccomodationController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(AccomodationUpVM objAccomodation, HttpPostedFileBase[] medDocFile, HttpPostedFileBase signFile)
        {
            ViewBag.GetMarriedAccnType = CustomDropDownList.GetMarriedAccnType();
            ViewBag.GetAccnArrangeType = CustomDropDownList.GetAccnArrangeType();
            ViewBag.GetAccnPreference = CustomDropDownList.GetAccnPreference();
            ...
            using (var uow = new UnitOfWork(new NDCWebContext()))
            {
                ...
                if (signFile != null && signFile.ContentLength > 0)
                {
                    ...
                    CheckBeforeUpload fs = new CheckBeforeUpload();
                    fs.filesize = 3000;
                    string result = fs.UploadFile(signFile);
                    if (string.IsNullOrEmpty(result))
                    {
                        UpdateDto.SignatureDoc = FilePath;
                        signFile.SaveAs(Server.MapPath(FilePath));
                    }
                    else
                        this.AddNotification(result, NotificationType.WARNING);
        ...
        [HttpPost]
        public async Task<ActionResult> Entry(AccomodationCrtVM objAccomodationvm, HttpPostedFileBase[] medDocFile, HttpPostedFileBase signFile)
        {
            string path = ServerRootConsts.ACCOMODATION_ROOT;
