...
namespace NDCWeb.Areas.Staff.Controllers
{
    ...
    public class TrainingActivityController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                //var indexDto = await uow.TrainingActivityRepo.GetTrainingActivityListAsync(0);
        ...
        public ActionResult Edit(TrainingActivityUpVM objTrgupvm, HttpPostedFileBase[] Files)
        {
            ...
            foreach (var file in Files)
            {
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    CheckBeforeUpload fs = new CheckBeforeUpload();
                    fs.filesize = 204800;
                    if (string.IsNullOrEmpty(fs.UploadFile(file)))
                    {
                        TrainingActivityMedia objMediaFile = new TrainingActivityMedia()
                        {
                            GuId = guid,
            ...
            using (var uow = new UnitOfWork(new NDCWebContext()))
            {
                ...
                //comment as on 17sep2021 by pradeep due to req. of old file as per DSCOORD
